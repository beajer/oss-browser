...
const base_crc64_addon_dir = path.join(
  __dirname,
  "../",
  "node/crc64/electron-crc64-prebuild/"
);

if (!fs.existsSync(path.join(base_crc64_addon_dir, "package.json"))) {
  fs.writeFileSync(path.join(base_crc64_addon_dir, "package.json"), "{}");
}
...
rebuild({
  ...
  shell.rm("-f", path.join(base_crc64_addon_dir, "package.json"));
